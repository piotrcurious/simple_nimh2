...
<body>
  <div id="profiler-wrap">
    ...
    <button data-cmd="charge">Start</button>
    <button data-cmd="ir">Measure IR</button>
    <button data-cmd="reset">Reset Ah</button>
    <button data-cmd="stop">Stop</button>
  </div>

  <div class="card">
    <div class="gaugesBar">
      <div class="gaugeCard"><canvas class="gauge" id="gaugeV"></canvas></div>
      <div class="gaugeCard"><canvas class="gauge" id="gaugeI"></canvas></div>
      <div class="gaugeCard"><canvas class="gauge" id="gaugeTD"></canvas></div>
      <div class="gaugeCard"><canvas class="gauge" id="gaugeDuty"></canvas></div>
    </div>
  </div>
</div>
...
<div class="grid">
  ...
  <div class="panel chargePanel fullWidth">
    ...
    const dom = {
      ...
      mainGraph: document.getElementById('mainGraph'),
      ambientGraph: document.getElementById('ambientGraph'),
      irGraph: document.getElementById('irGraph'),
      chargeGraph: document.getElementById('chargeGraph'),
      profilerCanvas: document.getElementById('gl'),
      profilerStatus: document.getElementById('profiler-status')
    };
    ...
    let lastMetricState = null;
    let lastMainData = null;
    let lastAmbientData = null;
    let lastIRData = null;
    let lastChargeData = [];
    let lastProfilerFrame = null;
    ...
    function drawSeries(ctx, arr, color, min, max, margin, label = '', lineWidth = 1) {
      ...
      let first = true;
      let lastX = 0, lastY = 0, lastVal = NaN;
    ...
    function renderCharge(data) {
      lastChargeData = Array.isArray(data) ? data : [];
      const canvas = dom.chargeGraph;
      const ctx = canvas.getContext('2d');
      const margin = { top: 12, right: 38, bottom: 16, left: 30 };
      fitCanvas(canvas);
      clearCanvas(ctx);

      if (!lastChargeData.length) {
        drawAxes(ctx, margin, 0, 100, 'Index', 0, 1, 'Value');
        dom.chargeLegend.textContent = '';
        return;
      }

      const arrI = lastChargeData.map(d => d.i);
      const arrV = lastChargeData.map(d => d.v);
      const arrD = lastChargeData.map(d => d.d);
      const arrTD = lastChargeData.map(d => d.td);
      const arrTH = lastChargeData.map(d => d.th);
      const arrIRLU = lastChargeData.map(d => d.irlu);
      const arrIRP = lastChargeData.map(d => d.irp);
      ...
      drawAxes(ctx, margin, 0, lastChargeData.length - 1, 'Index', yMin, yMax, 'Auto-scaled');
      ...
      const last = lastChargeData[lastChargeData.length - 1];
      dom.chargeLegend.textContent =
    ...
    function redrawFromCache() {
      if (lastMetricState) {
        drawAllGauges(lastMetricState);
      }
      if (lastMainData) {
        renderMain(lastMainData);
      }
      if (lastAmbientData) {
        renderAmbient(lastAmbientData);
      }
      if (lastIRData) {
        renderIR(lastIRData);
      }
      renderCharge(lastChargeData);
      if (lastProfilerFrame) {
        drawProfilerFrame(lastProfilerFrame);
      }
    }

    function updateMetricState(state) {
      lastMetricState = state;
      maxDT = Number.isFinite(state.max_dt) ? state.max_dt : 1.5;
      updateMetricText(state);
      drawAllGauges(state);
    }
    ...
    function readUint(ai, view, state) {
      ...
      if (ai === 25) {
        const v = view.getUint16(state.pos, false);
        state.pos += 2;
        return v;
      }
      if (ai === 26) {
        const v = view.getUint32(state.pos, false);
        state.pos += 4;
        return v;
      }
      if (ai === 27) {
    ...
    function decodeItem(view, state) {
      ...
      if (major === 7) {
        ...
        if (ai === 25) {
          const half = view.getUint16(state.pos, false);
          state.pos += 2;
        ...
        if (ai === 26) {
          const v = view.getFloat32(state.pos, false);
          state.pos += 4;
          return v;
        }
        if (ai === 27) {
          const v = view.getFloat64(state.pos, false);
          state.pos += 8;
          return v;
        }
    ...
    function initProfilerGL() {
      p_gl = dom.profilerCanvas.getContext('webgl', { antialias: false, alpha: false });
      if (!p_gl) {
        dom.profilerStatus.textContent = 'profiler WebGL unavailable';
        return;
      }

      try {
        const vs = compileShader(p_gl, p_gl.VERTEX_SHADER, `
        ...
        const fs = compileShader(p_gl, p_gl.FRAGMENT_SHADER, `
        ...
        if (!p_gl.getProgramParameter(p_program, p_gl.LINK_STATUS)) {
          throw new Error(p_gl.getProgramInfoLog(p_program) || 'Program link error');
        }
        ...
        dom.profilerStatus.textContent = 'profiler ready';
      } catch (err) {
        console.error(err);
        dom.profilerStatus.textContent = 'profiler init failed';
        p_gl = null;
      }
    }
    ...
    function drawProfilerFrame(frame) {
      if (!p_gl) return;
      lastProfilerFrame = frame;

      const w = dom.profilerCanvas.width;
      const h = dom.profilerCanvas.height;
      ...
      for (let core = 0; core < 2; core++) {
        ...
        for (const ev of frame.lanes[core]) {
          const x1 = (ev.startUs / period) * w;
          const x2 = ((ev.startUs + ev.durUs) / period) * w;
    ...
    function decodeProfilerFrame(arrayBuffer) {
      ...
      dom.profilerStatus.textContent = `frame ${dv.getUint32(4, true)} | C0:${c0} C1:${c1}`;
    }
    ...
    function connectWS() {
      ...
      ws.onclose = () => {
        ...
        dom.extraState.innerText = 'WebSocket reconnecting...';
    ...
    function handleWSData(data) {
      ...
      if (data.batch !== undefined) {
        ...
        lastChargeData.push(...data.batch);
        renderCharge(lastChargeData);
      }
    }
    ...
    function fitProfiler() {
      const dpr = window.devicePixelRatio || 1;
      const rect = dom.profilerCanvas.getBoundingClientRect();
      dom.profilerCanvas.width = Math.max(1, Math.round(rect.width * dpr));
      dom.profilerCanvas.height = Math.max(1, Math.round(rect.height * dpr));
    }

    function resizeAll() {
      fitProfiler();
      document.querySelectorAll('canvas.graph, canvas.gauge').forEach(canvas => fitCanvas(canvas));
      redrawFromCache();
    }
