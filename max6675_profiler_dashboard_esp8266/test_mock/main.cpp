...
#include "dummy_esp8266_server.h"
...
void test_live_telemetry() {
    std::cout << "Testing ESP8266 live telemetry serialization..." << std::endl;
...
void test_profiling_timeline_stream() {
    std::cout << "Testing ESP8266 binary profiling packet stream..." << std::endl;
...
void test_max6675_decoding() {
    std::cout << "Testing ESP8266 MAX6675 decoding logic with mock SPI stream..." << std::endl;

    mock_digitalRead_cb = mock_spi_digitalRead;

    // 1. Let's test a successful decoding of 350.5°C
    // raw temperature = 350.5 / 0.25 = 1402
    // MAX6675 format: bit 15: dummy, bit 14-3: raw temperature, bit 2: open input, bit 1: device ID, bit 0: state
...
int main() {
    ...
    std::cout << "All ESP8266 profile dashboard tests PASSED successfully!" << std::endl;
    return 0;
}
