...
void sendFramePacket(bool timeoutFlag) {
    ...
    // Snapshot counts and data quickly
    uint8_t c0 = g_coreBuf[0].count > MAX_EVENTS_PER_CORE ? MAX_EVENTS_PER_CORE : g_coreBuf[0].count;
    uint8_t c1 = g_coreBuf[1].count > MAX_EVENTS_PER_CORE ? MAX_EVENTS_PER_CORE : g_coreBuf[1].count;
