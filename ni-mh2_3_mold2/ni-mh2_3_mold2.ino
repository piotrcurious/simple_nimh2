...
void task_readThermistor(void* parameter) {
    ...
    while (true) {
        ...
        uint32_t now = millis();
        uint32_t time_elapsed = now - mAh_last_time;
    ...
    if (IrReceiver.decodedIRData.protocol == SAMSUNG && IrReceiver.decodedIRData.address == 0x7) {
        ...
        switch(IrReceiver.decodedIRData.command) {
            ...
            case RemoteKeys::KEY_MENU:
                ...
                currentDisplayState = DISPLAY_STATE_IDLE;} else {
                  currentDisplayState = DISPLAY_STATE_MAIN;
                }
...
void loop() {
    ...
    // --- Timed Events ---
    if (now - lastDataGatherTime >= PLOT_DATA_UPDATE_INTERVAL) { // gather data for main debug/status graph
        lastDataGatherTime = now;
        gatherData();
    }

        homeScreen.gatherData();   // gather data for idle/home screen (temp, humidity, dew point) 
...
//    if (currentDisplayState != DISPLAY_STATE_IDLE && now - displayStateChangeTime > 20000) {
//        currentDisplayState = DISPLAY_STATE_IDLE;
//        tft.fillScreen(TFT_BLACK);
//        displayStateChangeTime = now; 
//    }
