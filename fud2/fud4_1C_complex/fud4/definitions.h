...
#define THERMISTOR_VCC_OVERSAMPLING 256
...
#define CURRENT_SHUNT_OVERSAMPLING 32
#define PWM_PIN 19
...
#define PLOT_DATA_UPDATE_INTERVAL 2000
#define HIST_LOG_INTERVAL_MS 60000
...
const int MAX_RESISTANCE_POINTS = 120;
...
const float REMEASURE_MIN_CURRENT_DIFF = 0.02f;                 // Minimum current delta required to calculate local IR (A)
...
const float MAX_PULSE_CYCLE_LENGTH_S = 240.0f;                  // Maximum clamped duration for the combined charge pulse cycle (s)
...
const int STRATIFIED_PAIRS_SEGMENTS = 4;                        // Number of equal current segments for pairs stratified random sampling
...
const int PULSE_REMEASURE_BUDGET = 16;                         // Increased budget of re-measurements per pulse cycle
const float GLOBAL_PAIR_MIN_DELTA_I = 0.08f;                     // Min current delta for global pairs (large Delta I, stable baseline)
const float LOCAL_PAIR_MAX_DELTA_I = 0.08f;                     // Max current delta for local pairs (close currents, local accuracy)
const float LOCAL_PAIR_MIN_DELTA_I = 0.02f;                     // Min current delta for local pairs
...
//static constexpr uint32_t FRAME_PERIOD_US = 200000;   // 5 Hz
