...
void storeOrAverageResistanceData(float current, float resistance, float data[][2], int& count) {
    ...
    if (count < MAX_RESISTANCE_POINTS) {
        ...
        // Array is full - merge with closest point
        ...
        // Calculate isolation threshold based on data spacing
        float isolationThreshold = 0.02f; // Default minimum threshold
        ...
        if (count >= 2) {
            ...
            if (!spacings.empty()) {
                ...
                // Threshold is mean + 1.5 * std dev
                isolationThreshold = max(0.02f, meanSpacing + 1.5f * stdDevSpacing);
            }
        }
        ...
        if (count > 1) {
            float distanceToPrev = (closestIndex > 0) ? 
                fabs(data[closestIndex][0] - data[closestIndex - 1][0]) : isolationThreshold * 2.0f;
            float distanceToNext = (closestIndex < count - 1) ? 
                fabs(data[closestIndex][0] - data[closestIndex + 1][0]) : isolationThreshold * 2.0f;

            isIsolated = (distanceToPrev >= isolationThreshold && distanceToNext >= isolationThreshold);
        } else {
            isIsolated = false; // Single point cannot be isolated
        }
        ...
        if (isIsolated && count >= 2) {
            ...
            if (index1 != -1 && index2 != -1) {
                ...
                // Find new insertion position and insert
                int insertIndex = 0;
                while (insertIndex < count && data[insertIndex][0] < current) {
                    insertIndex++;
                }
                ...
                // Only one neighbor - just average with closest point
    ...
    for (int i = 0; i <= count - 4; ++i) {
        for (int j = i + 3; j < count; ++j) {
            ...
            if (clusterSpan <= spacing_threshold) {
                // Found a cluster of at least 4 points
