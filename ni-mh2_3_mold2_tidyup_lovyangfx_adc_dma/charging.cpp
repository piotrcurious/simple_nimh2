...
// Global helper: accumulated dissipated energy not yet applied to the thermal model (Joules)
static float g_unappliedEnergy_J = 0.0f;
// How quickly the internal reservoir releases energy into the cell (seconds).
// Tune this to match your cell's internal equilibration speed.
// Default: 10 s (reasonable starting point; make it larger if release is slower).
static float g_internalReleaseTau_s = 60.0f;

// Helpers
static void addUnappliedEnergy(float energyJ) {
  if (energyJ > 0.0f) g_unappliedEnergy_J += energyJ;
}
static float readUnappliedEnergy(void) {
  return g_unappliedEnergy_J;
}
static void setInternalReleaseTau(float tau_s) {
  if (tau_s > 0.0f) g_internalReleaseTau_s = tau_s;
}
static float getInternalReleaseTau(void) {
  return g_internalReleaseTau_s;
}
...
float estimateTempDiff(
  ...
  float G = thermalConductance_W_per_K(area, convectiveH, emissivity, T_amb_K); // W/K
  float Cth = cellMassKg * specificHeat; // J/K

  const float tiny = 1e-12f;
  float theta0 = BatteryTempC - ambientTempC; // initial relative temperature (K)

  // Energy generated during this interval (J)
  float E_generated_J = P * dt_s;

  // --- Release from internal energy reservoir (exponential release) ---
  float E_released_J = 0.0f;
  if (g_unappliedEnergy_J > 0.0f) {
    // release fraction during dt: 1 - exp(-dt/τ_rel)
    ...
    // subtract released energy from reservoir (clamped)
    g_unappliedEnergy_J -= E_released_J;
    if (g_unappliedEnergy_J < 0.0f) g_unappliedEnergy_J = 0.0f;
  }

  // Total energy that actually entered the thermal node during dt:
  float E_input_total_J = E_generated_J + E_released_J;

  // If thermal conductance is essentially zero: no heat lost to ambient.
  // Convert all energy input during dt directly to ΔT.
  if (G <= tiny || Cth <= tiny) {
    float deltaT = E_input_total_J / ( (Cth <= tiny) ? 1.0f : Cth );
    return theta0 + deltaT;
  }

  // For G>0 use lumped-capacitance analytic solution.
  // We'll use the average power over dt: P_avg = E_input_total_J / dt_s.
  // This ensures energy conservation (E_input_total_J is exactly the energy added).
  float P_avg = E_input_total_J / dt_s;

  float theta_ss = P_avg / G;   // steady-state relative temperature corresponding to average power
  float tau = Cth / G;          // thermal time constant (s)
