...
void handleWebSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
    switch(type) {
        ...
        case WStype_CONNECTED: {
            ...
            Serial.printf("[%u] Connected from %d.%d.%d.%d url: %s\n", num, ip[0], ip[1], ip[2], ip[3], (char*)payload);

            // Send initial state and ambient
            WEB_LOCK();
            CborWriter w;
            w.reserve(512);
            w.startMap(2);
            ...
            w.addText("ambient"); appendCborAmbient(w);
            WEB_UNLOCK();
            webSocket.sendBIN(num, w.data.data(), w.data.size());
            break;
        }
        case WStype_TEXT: {
...
// Function to be called periodically (e.g. from task_updateSystemData or a new task)
// to broadcast live telemetry.
void broadcastLiveTelemetry() {
    ...
    webSocket.broadcastBIN(w.data.data(), w.data.size());
}
#endif
