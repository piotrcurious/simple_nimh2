...
bool chargeBattery() {
    ...
    if (dataStore.chargingState != CHARGE_IDLE && dataStore.chargingState != CHARGE_STOPPED && (now - dataStore.chargingStartTime >= TOTAL_TIMEOUT)) {
        Serial.println("Charging timeout, stopping charging for safety reasons.");
        dataStore.chargingState = CHARGE_STOPPED;
    }

    switch (dataStore.chargingState) {
        case CHARGE_IDLE: {
            Serial.println("Starting battery charging process...");
            dataStore.chargingStartTime = now;
            dataStore.lastChargeEvaluationTime = now;
            dataStore.overtemp_trip_counter = 0;
            ...
            startLog.internalResistanceLoadedUnloaded = dataStore.regressedInternalResistanceIntercept;
            startLog.internalResistancePairs = dataStore.regressedInternalResistancePairsIntercept;
            ...
            dataStore.chargingState = CHARGE_FIND_OPT;
            break;
        }
        case CHARGE_FIND_OPT: {
            if (!findOptimalChargingDutyCycleStepAsync()) {
                ...
                dataStore.dutyCycle = appliedDC;
                analogWrite(PWM_PIN, dataStore.dutyCycle);
                lastOptimalDutyCycle = dataStore.dutyCycle;
                dataStore.lastChargeEvaluationTime = now;
...
/*  // do not log after applying optimal duty cycle as this is not final current and voltage of each charging cycle after evaluation
                ...
                entry.dutyCycle = dataStore.dutyCycle;
                entry.internalResistanceLoadedUnloaded = dataStore.regressedInternalResistanceIntercept;
                entry.internalResistancePairs = dataStore.regressedInternalResistancePairsIntercept;
                logChargeData(entry);
*/

                dataStore.chargingState = CHARGE_MONITOR;
            }
            break;
        }
        ...
        case CHARGE_MONITOR: {
            ...
            float currentRampTarget = 0.0f; // This should be in DataStore if it needs to persist
            if (current > currentRampTarget) {
              if (dataStore.dutyCycle > 0){
                dataStore.dutyCycle--;
                analogWrite(PWM_PIN, dataStore.dutyCycle);
              }
            }

            if (now - dataStore.lastChargeEvaluationTime >= CHARGE_EVALUATION_INTERVAL_MS) {
                float maximumCurrent = 0.150; // This should be in DataStore if it needs to persist
                ...
                float tempRise_relative = estimateTempDiff(voltage, voltage, current,
                                                  ...
                                                  temp1, now, dataStore.lastChargeEvaluationTime, temp2);
                Serial.print("Estimated temperature rise due to Rint heating (relative): ");
                Serial.print(tempRise_relative);
                Serial.println(" °C");


            //    dataStore.MAX_DIFF_TEMP = (MAX_TEMP_DIFF_THRESHOLD + tempRise);
...
// --- compute absolute temprise by replaying history ---
    ...
    float MAX_DIFF_TEMP = (MAX_TEMP_DIFF_THRESHOLD + finalTempRise);
...
void startCharging() {
    if (dataStore.app_state != APP_STATE_CHARGING) {
        dataStore.app_state = APP_STATE_CHARGING;
        dataStore.chargingState = CHARGE_IDLE;
