...
/**
 * CBOR STREAM WRITER
 * Streams directly to the HTTP response in small chunks.
 * This avoids buffering the full payload in RAM.
 */
struct CborStreamWriter {
    static constexpr size_t BUF_SIZE = 256;
    uint8_t buf[BUF_SIZE];
    size_t pos = 0;

    void reserve(size_t) {} // kept for compatibility with previous code

    void flush() {
        if (pos > 0) {
            server.sendContent((const char*)buf, pos);
            pos = 0;
        }
    }
...
static void cborAddFloatArray(CborStreamWriter& w, const float* arr, int len) {
    w.startArray(len);
    for (int i = 0; i < len; i++) {
...
static void cborAddXYPairs(CborStreamWriter& w, float data[][2], int count) {
    w.startArray(count);
...
/**
 * STREAMING RESPONSE STARTER
 */
static void beginCborResponse() {
 ...
 * CBOR Data Builders
 */
static void appendCborState(CborStreamWriter& w) {
    extern float noiseFloorMv;
    w.startMap(10);
...
static void appendCborChargeLog(CborStreamWriter& w) {
